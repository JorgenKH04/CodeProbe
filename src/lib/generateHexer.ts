...
const defaultLowerColorLib = ['#E96B89', '#F36F52', '#F2994A', '#23da35', '#92CCC9', '#CDD9F0', '#92A2CC', '#A892CC'];
...
export const generateHexer = (props: {
  ...
  const { upperPath, upperRotation } = (() => {
    const random = seedrandom(name);
    const points = [new Vec2(0.5, -0.5), new Vec2(0.2, 1.2), new Vec2(0.8, 1.2)].map((point) =>
      point.add(new Vec2(random() - 0.5, random() - 0.5).multiply(0.5)).multiply(100),
    );

    return {
      upperPath: name ? `M${points[0].x} ${points[0].y} L${points[1].x} ${points[1].y} L${points[2].x} ${points[2].y} z` : '',
